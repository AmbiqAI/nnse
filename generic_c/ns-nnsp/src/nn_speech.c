...
int32_t embd_nnid[64 * 5] = {
    ...
    -407,  2522,  4440,  -2831, -1328, 1913,  2945,  6214,  1257,  -6725, -4230, -2121, 1773,
    3555,  -1782, -1663, -1325, 3906,  3340,  2317,  -2386, -3092, 2364,  -4930, 4128};
...
int NNSPClass_free(NNSPClass *pt_inst)
{
    FeatureClass_destruct((FeatureClass *)pt_inst->pt_feat);
    return 0;
}
...
int16_t NNSPClass_exec(NNSPClass *pt_inst, int16_t *rawPCM) {
...
#else
        int32_t gain = ((int32_t)pt_inst->pt_params->pre_gain_q1);
        for (int i = 0; i < 160; i++) {
            int32_t tmp = (int32_t)rawPCM[i] * gain;
            tmp >>= 1;
            if (tmp > INT16_MAX) tmp = INT16_MAX;
            else if (tmp < INT16_MIN) tmp = INT16_MIN;
            rawPCM[i] = (int16_t)tmp;
        }
#endif
    ...
    if (pt_inst->slides == 1) {
        ...
        case kws_galaxy_id:
            binary_post_proc(pt_inst, glob_nn_output, &pt_inst->trigger);
            break;
...
#if ARM_FFT==0
    stftModule_synthesize(pt_stft_state, spec, pt_se_out);
#else
...
void binary_post_proc(NNSPClass *pt_inst, int32_t *pt_est, int16_t *pt_trigger) {
    int32_t max_val;
    int32_t i;
