...
void dmSecHciHandler(hciEvt_t *pEvent)
{
  ...
  if ((pCcb = dmConnCcbByHandle(pEvent->hdr.param)) != NULL)
  {
             ...
             pEvent->hdr.event == HCI_ENC_CHANGE_CBACK_EVT)
    {
      ...
      dmEncryptInd.encryptInd.hdr.param = pCcb->connId;
      dmEncryptInd.encryptInd.hdr.status = pEvent->hdr.status;
      if (dmEncryptInd.encryptInd.hdr.status == HCI_SUCCESS)
      {
        dmEncryptInd.encryptInd.hdr.event = DM_SEC_ENCRYPT_IND;

        /* update security level of connection */
        pCcb->secLevel = pCcb->tmpSecLevel;

        /* set LTK flag */
        dmEncryptInd.encryptInd.usingLtk = pCcb->usingLtk;
      }
      else
      {
        dmEncryptInd.encryptInd.hdr.event = DM_SEC_ENCRYPT_FAIL_IND;
      }

      /* call callback before passing to SMP */
      DmSmpCbackExec(&dmEncryptInd);

      /* pass to SMP */
      dmEncryptInd.encryptInd.hdr.param = pCcb->connId;
      dmEncryptInd.encryptInd.hdr.status = pEvent->hdr.status;
      SmpDmEncryptInd((wsfMsgHdr_t *) &dmEncryptInd);
    }
  }
}
