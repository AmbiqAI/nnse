...
class stft_class: # pylint: disable=invalid-name
    ...
    def istft_frame_proc(
        ...
        istft_frame_proc
        """
        data_freq = data_freqs[-1].copy()
        tfmask = np.reshape(tfmask,(4,-1))
        amp_data_freqs = np.abs(data_freqs)
        ave_amp = np.mean(amp_data_freqs * np.maximum(tfmask, min_tfmask), 0)
        phase = data_freq / (np.abs(data_freq))
