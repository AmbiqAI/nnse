...
def c_matrix_man(
        mat,
        arm_core="M4", # 0: 2macs, 1: M4, 2: M55-no-reshpae
    ...
    for i in range(blks_r): # case matrix bks in 4xN
        ...
        mat_reshape = reshape_mat_KxN(sub_mat, mat_reshape, arm_core)
...
def c_lstm_weight_man(
    ...
    kernel_fw_reshape  = c_lstm_kernel_man(kernel_fw, rows, arm_core)
    kernel_re_reshape = c_lstm_kernel_man(kernel_re, rows, arm_core)
    bias_reshape  = c_lstm_bias_man(bias, rows)

    return kernel_fw_reshape, kernel_re_reshape, bias_reshape

def c_lstm_kernel_man(
        kernel,
        rows,
        arm_core='M4', # 0: 2macs, 1: M4, 2: M55-no-reshpae
        ):
    ...
    for i in range(blks_r):
        ...
        kernel_reshape = np.concatenate((kernel_reshape, c_matrix_man(i_kernel, arm_core)))
        kernel_reshape = np.concatenate((kernel_reshape, c_matrix_man(j_kernel, arm_core)))
        kernel_reshape = np.concatenate((kernel_reshape, c_matrix_man(f_kernel, arm_core)))
        kernel_reshape = np.concatenate((kernel_reshape, c_matrix_man(o_kernel, arm_core)))
