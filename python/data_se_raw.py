...
import librosa
from nnsp_pack import tfrecord_converter_se_rawspeech
            ...
            if success:
                ...
                if reverbing:
                    tfrecord = re.sub(  r'\.tfrecord$',
                                        f'_snr{snr_db}dB_{ntype}_reverb_raw.tfrecord',
                                        tfrecord)
                else:
                    tfrecord = re.sub(  r'\.tfrecord$',
                                        f'_snr{snr_db}dB_{ntype}_raw.tfrecord',
                                        tfrecord)
                os.makedirs(os.path.dirname(tfrecord), exist_ok=True)
                try:
                    tfrecord_converter_se_rawspeech.make_raw_tfrecord( # pylint: disable=too-many-function-args
                                        tfrecord,
                                        audio_sn,
                                        audio_s)
...
def main(args):
    ...
    if not DEBUG:
        for train_set in sets_categories:
            with open(f"data/{train_set}_tfrecords_se_raw.csv", 'w') as file: # pylint: disable=unspecified-encoding
