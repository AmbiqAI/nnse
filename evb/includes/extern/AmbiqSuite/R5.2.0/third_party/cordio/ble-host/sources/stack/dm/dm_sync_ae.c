...
void dmSyncReset(void)
{
  ...
  syncLost.lePerAdvSyncLost.hdr.event = HCI_LE_PER_ADV_SYNC_LOST_CBACK_EVT;
  syncLost.lePerAdvSyncLost.hdr.status = HCI_SUCCESS;
  ...
  for (i = DM_SYNC_MAX; i > 0; i--, pScb++)
  {
    ...
    {
      /* set sync handle */
      syncLost.lePerAdvSyncLost.hdr.param = syncLost.lePerAdvSyncLost.syncHandle = pScb->handle;

      /* handle the event */
      dmSyncHciHandler(&syncLost);
    }
  }
}
